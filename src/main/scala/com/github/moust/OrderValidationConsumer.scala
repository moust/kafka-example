...
object OrderValidationConsumer extends App {
  ...
  scheduler.schedule(
    () => {
      ...
      val serdeGenerator       = SerdeGenerator.fromUrl("http://localhost:8081/")
      val orderValidationSerde = serdeGenerator.serdeFor[OrderValidation]
      ...
      Try(consumer.close()).recover { case error =>
        logger.error("Failed to close the kafka consumer", error)
      }
